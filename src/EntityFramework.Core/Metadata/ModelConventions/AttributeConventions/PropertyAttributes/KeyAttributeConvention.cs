...
namespace Microsoft.Data.Entity.Metadata.ModelConventions.AttributeConventions.PropertyAttributes
{
    public class KeyAttributeConvention : PropertyAttributeConvention<KeyAttribute>, IModelConvention
    {
        public override void Apply(InternalPropertyBuilder propertyBuilder, KeyAttribute attribute)
        {
            ...
            newKey = entityTypeBuilder.PrimaryKey(properties, ConfigurationSource.DataAnnotation);
            if (newKey != null)
            {
                properties = currentKey.Properties.Select(p => p.Name).ToList();
                properties.Add(propertyBuilder.Metadata.Name);
                entityTypeBuilder.PrimaryKey(properties, ConfigurationSource.DataAnnotation);
            }
        }
